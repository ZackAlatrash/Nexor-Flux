...
// Baseline-profile consumer config. Kept inert for normal builds: generation must be an explicit
// manual/CI step (it launches a device/emulator), never triggered by a plain assemble/build.
//   Generate with a connected device or GMD:  ./gradlew :app:generateReleaseBaselineProfile
baselineProfile {
    // Do NOT (re)generate the profile during ordinary builds — otherwise assembleRelease would try
    // to spin up a device. Generation stays opt-in via the generate*BaselineProfile tasks.
...
dependencies {
    ...
    // Consumes the baseline profile generated by the :macrobenchmark module. The profile is compiled
    // into the release/benchmark APKs (AOT-precompiling hot startup code) via the androidx
    // .baselineprofile plugin. Generation is a manual/CI step (see baselineProfile { } below).
