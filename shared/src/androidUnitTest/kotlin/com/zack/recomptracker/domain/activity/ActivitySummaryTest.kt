...
class ActivitySummaryTest {
    ...
    private val today = LocalDate(2026, 6, 30)

    @Test
    fun `workoutDays unions sessions and trained logs, de-duplicating`() {
        val sessions = listOf(LocalDate(2026, 6, 29), LocalDate(2026, 6, 30))
        val trained = listOf(LocalDate(2026, 6, 30), LocalDate(2026, 6, 28))
        assertEquals(
            setOf(LocalDate(2026, 6, 28), LocalDate(2026, 6, 29), LocalDate(2026, 6, 30)),
    ...
    fun `weekly training frequency averages distinct days over the window`() {
        ...
        val days = (0 until 8).map { today.minus(it * 2, DateTimeUnit.DAY) }.toSet()
    ...
    fun `weekly training frequency ignores days outside the window`() {
        val inside = today.minus(3, DateTimeUnit.DAY)
        val outside = today.minus(40, DateTimeUnit.DAY) // beyond 4 weeks
        assertEquals(
            0.25, // 1 day / 4 weeks
    ...
    fun `average steps means the trailing window and rounds`() {
        val steps = mapOf(
            today to 10_000,
            today.minus(1, DateTimeUnit.DAY) to 5_000,
            today.minus(2, DateTimeUnit.DAY) to 6_001, // sum 21001 / 3 = 7000.33 -> 7000
        )
    ...
    fun `average steps excludes days outside the window`() {
        val steps = mapOf(
            today to 9_000,
            today.minus(10, DateTimeUnit.DAY) to 1_000, // outside a 7-day window
        )
    ...
    fun `average steps is null when nothing logged in window`() {
        ...
        assertNull(
            ActivitySummary.averageDailySteps(mapOf(today.minus(30, DateTimeUnit.DAY) to 8000), today, days = 7),
        )
    }
}
