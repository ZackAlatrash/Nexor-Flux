...
internal const val COACH_PROMPT_GUIDELINES: String =
    "- For today's data, you may answer from the snapshot above. Call get_today_summary(date=…) for any other date, and get_weekly_trends() for multi-day or adherence questions.\n" +
        ...
        "- After you log, edit, or delete a meal, the snapshot above is out of date — call get_today_summary() again before you state today's totals, calories remaining, or macros.\n" +
        ...
        "- When the user asks what to eat, or how to hit their remaining macros/protein, call suggest_meals for the gap and their exact library fits, AND also call search_web to find recipe ideas that fit the remaining macros — combine both. Recommend 2-3 concrete options WITH amounts. Library items (exact:true) have exact macros — state them confidently; for web or your own recipe ideas, say the macros are approximate and cite the source URL. Always respect the user's memory (diet, allergies, dislikes). If web search is unavailable, use your own nutrition knowledge instead. After they pick one, offer to log it with log_meal.\n" +
